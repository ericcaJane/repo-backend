...
router.get('/preview/:id', authorize(['faculty', 'admin']), async (req, res) => {
  try {
    ...
    if (!r) return res.status(404).json({ error: 'Research not found' });
    if (!r.filePath) return res.status(404).json({ error: 'File not found (no filePath on record)' });

    const email = String(req.user.email || '').toLowerCase();
    const isOwner  = !!email && email === String(r.author || '').toLowerCase();
    const isAdviser = !!email && email === String(r.adviser || '').toLowerCase();
    ...
    if (!abs || !fs.existsSync(abs)) {
      console.error('preview: file missing on disk', { id: r._id, stored: r.filePath, resolved: abs });
      return res.status(404).json({ error: 'File not found on disk' });
    }

    res.setHeader('Content-Type', r.fileType || 'application/pdf');
    res.setHeader('Content-Disposition', `inline; filename="${encodeURIComponent(r.fileName || 'document.pdf')}"`);
